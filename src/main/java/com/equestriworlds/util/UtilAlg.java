...
public class UtilAlg {
    ...
    public static Vector getTrajectory(Entity from, Entity to) {
        return UtilAlg.getTrajectory(from.getLocation().toVector(), to.getLocation().toVector());
    }

    public static Vector getTrajectory(Location from, Location to) {
        return UtilAlg.getTrajectory(from.toVector(), to.toVector());
    }
    ...
    public static Vector getTrajectory2d(Entity from, Entity to) {
        return UtilAlg.getTrajectory2d(from.getLocation().toVector(), to.getLocation().toVector());
    }

    public static Vector getTrajectory2d(Location from, Location to) {
        return UtilAlg.getTrajectory2d(from.toVector(), to.toVector());
    }
    ...
    public static boolean HasSight(Location from, Player to) {
        return UtilAlg.HasSight(from, to.getLocation()) || UtilAlg.HasSight(from, to.getEyeLocation());
    }
    ...
    public static boolean HasSight(Location from, Location to) {
        ...
        double rate = 0.1;
        Vector vec = UtilAlg.getTrajectory(from, to).multiply(0.1);
        while (UtilMath.offset(cur, to) > rate) {
            cur.add(vec);
            if (UtilBlock.airFoliage(cur.getBlock())) continue;
            return false;
        }
        return true;
    }
    ...
    public static float GetPitch(Vector vec) {
        ...
        pitch = y <= 0.0 ? (pitch += 90.0) : (pitch -= 90.0);
        if (pitch == 180.0) {
            pitch = 0.0;
        }
        return (float)pitch;
    ...
    public static Vector getRight(Vector vec) {
        return UtilAlg.cross(vec.clone().normalize(), new Vector(0, 1, 0));
    }

    public static Vector getLeft(Vector vec) {
        return UtilAlg.getRight(vec).multiply(-1);
    }
    ...
    public static Vector getUp(Vector vec) {
        return UtilAlg.getDown(vec).multiply(-1);
    }

    public static Vector getDown(Vector vec) {
        return UtilAlg.cross(vec, UtilAlg.getRight(vec));
    }
    ...
    public static Location findClosest(Location mid, ArrayList<Location> locs) {
        ...
        for (Location loc : locs) {
            ...
            if (bestLoc != null && dist >= bestDist) continue;
            bestLoc = loc;
            bestDist = dist;
        }
        return bestLoc;
    ...
    public static boolean isInPyramid(Vector a, Vector b, double angleLimit) {
        return (double)Math.abs(UtilAlg.GetPitch(a) - UtilAlg.GetPitch(b)) < angleLimit && (double)Math.abs(UtilAlg.GetYaw(a) - UtilAlg.GetYaw(b)) < angleLimit;
    }

    public static boolean isTargetInPlayerPyramid(Player player, Player target, double angleLimit) {
        return UtilAlg.isInPyramid(player.getLocation().getDirection(), UtilAlg.getTrajectory(player.getEyeLocation(), target.getEyeLocation()), angleLimit) || UtilAlg.isInPyramid(player.getLocation().getDirection(), UtilAlg.getTrajectory(player.getEyeLocation(), target.getLocation()), angleLimit);
    }
    ...
    public static Location getLocationAwayFromPlayers(ArrayList<Location> locs, ArrayList<Player> players) {
        ...
        for (Location loc : locs) {
            ...
            for (Player player : players) {
                ...
                if (closest != -1.0 && dist >= closest) continue;
                closest = dist;
            }
            if (closest == -1.0 || bestLoc != null && closest <= bestDist) continue;
            bestLoc = loc;
            bestDist = closest;
        }
        return bestLoc;
    ...
    public static Location getLocationNearPlayers(ArrayList<Location> locs, ArrayList<Player> players, ArrayList<Player> dontOverlap) {
        ...
        for (Location loc : locs) {
            ...
            for (Player player : dontOverlap) {
                if (!player.getWorld().equals((Object)loc.getWorld()) || (dist = UtilMath.offsetSquared(player.getLocation(), loc)) >= 0.8) continue;
                valid = false;
                break;
            }
            if (!valid) continue;
            for (Player player : players) {
                if (!player.getWorld().equals((Object)loc.getWorld())) continue;
                dist = UtilMath.offsetSquared(player.getLocation(), loc);
                if (closest != -1.0 && dist >= closest) continue;
                closest = dist;
            }
            if (closest == -1.0 || bestLoc != null && closest >= bestDist) continue;
